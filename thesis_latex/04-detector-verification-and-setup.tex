...
\texttt{backbone.} prefix (for example \texttt{encoder.0.0.conv1.weight} rather
than \texttt{backbone.encoder.0.0.conv1.weight}). It then called
\texttt{load\_state\_dict(state, strict=False)} on this raw model. Because
...
(Chapter~\ref{chap:main-results}, Section~5.5), and that role requires only that the
